...
class MolGNN(nn.Module):
    ...
    def forward(self, batch: Batch):
        # Here, instead of using actual input node features, we assign every node in the graph
        # the exact same learnable embedding (self.node_init). This is done by first unsqueezing
        # self.node_init to shape (1, hidden), then expanding it to (num_nodes, hidden), where
        # num_nodes is the total number of nodes in the batch. Thus, h will be a tensor where
        # every row is identical and trainable, serving as the initial node feature matrix.
